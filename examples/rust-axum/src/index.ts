...
export class RustAxumContainer extends Container {
...
export default {
  async fetch(
    request: Request,
    env: { RUST_AXUM_CONTAINER: DurableObjectNamespace<RustAxumContainer> }
  ): Promise<Response> {
    ...
    if (pathname.startsWith('/container')) {
      const containerInstance = getContainer(env.RUST_AXUM_CONTAINER, pathname);
      return containerInstance.fetch(request);
    }

    if (pathname.startsWith('/error')) {
      const containerInstance = getContainer(env.RUST_AXUM_CONTAINER, 'error-test');
      return containerInstance.fetch(request);
    }

    if (pathname.startsWith('/lb')) {
      const containerInstance = await getRandom(env.RUST_AXUM_CONTAINER, 3);
      return containerInstance.fetch(request);
    }

    if (pathname.startsWith('/singleton')) {
      // getContainer will return a specific instance if no second argument is provided
      return getContainer(env.RUST_AXUM_CONTAINER).fetch(request);
    }
